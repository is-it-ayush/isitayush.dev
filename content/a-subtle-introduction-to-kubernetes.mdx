...
To give you an analogy imagine a master worker updating it's
job book and writing, _"Computer 5's job is to run 3 copies of
this web server container"_. Workers computers simply look at
the job book present on master computers, figure out what
their job is and attempt to do it. This is just a simplified
way to explain how kubernetes works. Sometimes **computers
with the master role are also called control-plane nodes and
computers with the worker role are also called worker nodes.**
...
2. Create a new virtual machine in virt-manager so it opens
   ...
   5. You can name your virtual machine anything you want.
      ...
      > after the image is created and make a backup so you can create
      > more clusters in you make a mistake and mess up.
...
the path of image from "Storage: ..." at step `2` when we created this virtual
machine. You'll find this image there and you can backup this image
somewhere safe so you create clusters to your hearts content or to
initialize a new one in case you mess up.
...
--discovery-token-ca-cert-hash sha256:...`
   command and save it somewhere. We'll use this command to join computers
   as workers to our cluster.
...
11. We can now join other two virtual machines as workers
    ...
    earlier during cluster creation on them. A successful run would
    output this,
