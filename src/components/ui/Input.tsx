...
interface InputProps extends Omit<React.HTMLProps<HTMLInputElement>, "style">, VariantProps<typeof InputStyles> {
...
export const Input = (props: InputProps) => {
    ...
    return (
        <div className={cn("flex h-14 flex-col space-y-2", className)}>
            <div
                className={cn(
                    error && error?.length > 0 ? InputStyles({style: "red"}) : InputStyles({style}),
            ...
            {error && error.length > 0 && <span className="text-xs text-red-900 dark:text-red-700">{error}</span>}
        </div>
    );
};
