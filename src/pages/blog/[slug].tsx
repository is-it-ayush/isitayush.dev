...
export default function Entry({
  ...
  // POST | comment mutation | /api/comment/create | { comment: string, slug: string }
  const postCommentMutation = useMutation({
    mutationFn: (data: CreateCommentSchema): Promise<ApiError<boolean>> =>
      postData(`/api/comment/create`, data),
  });
      ...
      <article className="prose flex flex-col min-w-[300px] mt-10 lg:max-w-[500px] gap-4">
        <NextSeo
          ...
          openGraph={{
            ...
            images: [
              {
                url: new URL(
                  `${url}/api/og?title=${data?.entry?.title}&date=${data?.entry?.publishedAt
                    ? format(
        ...
        <div className="flex flex-col gap-6">
          <div className="flex flex-row justify-between">
            ...
            <Button
              tooltip={`sort comments by ${sortOrder === "asc" ? "oldest" : "newest"} first.`}
              className="font-light p-2"
              onClick={() => {
                setSortOrder(sortOrder === "asc" ? "desc" : "asc");
              }}>
              {sortOrder === "asc" ? <ArrowDown01 className="w-4 h-4" /> : <ArrowUp10 className="w-4 h-4" />}
            </Button>
          </div>
          <div className="flex flex-row gap-2">
                  ...
                  <div className="flex flex-row gap-4 items-center w-full">
                    ...
                    <div className="flex flex-col gap-2 w-full text-sm">
                      <div className="flex flex-row gap-4 justify-between items-center">
                        ...
                        <div className="flex flex-row gap-2">
                          <Button type="submit" className="font-light" tooltip="post comment.">
                            comment.
                          </Button>
                        </div>
                      </div>
          ...
          <motion.ul variants={commentListAnimation} initial="hidden" animate="visible" className="flex flex-col gap-4 w-full text-sm list-none p-0 m-0">
            {postsQuery.data?.data?.comments?.sort((a, b) => {
